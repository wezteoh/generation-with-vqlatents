"""Various sampling methods based on the score SDE paper."""
...
import src.modules.sde as sde_lib
...
def from_flattened_numpy(x: np.ndarray, shape: tuple[int, ...]) -> torch.Tensor:
    return torch.tensor(x, device="cpu", dtype=torch.float32).reshape(shape)


def get_score_fn(sde, model, train: bool, continuous: bool):
    # `model` is expected to accept `model(x, sigmas=...)`.
    def score_fn(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        # Continuous-time score uses the marginal std at time t.
        if continuous:
            std = sde.marginal_prob(x, t)[1]
        else:
            # If discrete sigmas exist, use them; otherwise fall back to marginal std.
            if hasattr(sde, "discrete_sigmas"):
                timestep = (t * (sde.N - 1) / sde.T).long()
                std = sde.discrete_sigmas.to(t.device)[timestep]
            else:
                std = sde.marginal_prob(x, t)[1]
        return model(x, sigmas=std)
...
def get_pc_sampler(
    ...
    def pc_sampler(model):
        ...
        with torch.no_grad():
            ...
            # Return samples directly (no inverse data normalizer).
