        ...
        for _ in range(n_blocks):
            blocks.append(
                _SimpleResidualBlock(
                    out_channels, out_channels, resample=None, act=self.act
                )
            )
...
class _NCSNv2Backbone(nn.Module):
    ...
    def __init__(
        ...
        self.res2 = nn.ModuleList(
            [
                _SimpleResidualBlock(
                    self.ngf, 2 * self.ngf, resample="down", act=self.act
                ),
                _SimpleResidualBlock(
                    2 * self.ngf, 2 * self.ngf, resample=None, act=self.act
                ),
            ]
        )
        ...
        self.refine1 = _RefineBlock(
            [2 * self.ngf], 2 * self.ngf, act=self.act, start=True
        )
        self.refine2 = _RefineBlock(
            [2 * self.ngf, 2 * self.ngf], 2 * self.ngf, act=self.act
        )
        self.refine3 = _RefineBlock(
            [2 * self.ngf, 2 * self.ngf], self.ngf, act=self.act
        )
        self.refine4 = _RefineBlock(
            [self.ngf, self.ngf], self.ngf, act=self.act, end=True
        )
