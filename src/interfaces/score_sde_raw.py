...
from src.interfaces.score_sde_latent import _append_dims
...
class ScoreSDERawInterface(pl.LightningModule):
    """Lightning wrapper for training a score network over raw image SDE trajectories.

    Training perturbs the clean image with the SDE marginal:
      x_t = mean(x_0, t) + std(t) * z, z ~ N(0, I)
    ...
    def __init__(
        ...
        # Raw score model: operate directly on (B,C,H,W) tensors (no VQ first-stage).
        ...
        if self.continuous:
            ...
            if not hasattr(self.sde, "discrete_sigmas"):
                raise NotImplementedError(
                    "Non-continuous ScoreSDE training requires an SDE with `discrete_sigmas` "
